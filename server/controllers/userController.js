...
const postRegistration = async (req,res,next) => {
    try {
        ...
        return res.status(201).json(createdUserObject(user.id,user.email))
    } catch (error) {
        return next (error)
    }
}

const createdUserObject = (id,email,token=undefined) => {
    return {
        'id': id,
        'email': email,
...
const postLogin = async (req,res,next) => {
    ...
    try {
        ...
        return res.status(200).json(createdUserObject(user.id,user.email,token))
    } catch (error) {
        return next(error)
    }
}
